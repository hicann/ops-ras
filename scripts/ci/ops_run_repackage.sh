...
MERGE_SCRIPT="${WORKDIR}/scripts/package/ops_ras/scripts/merge_binary_info_config.py"
...
host_file_name=$(find . -type f -name "*.run" | grep -v "custom" | grep "ops-ras")
...
for runfile in kernel_*.run; do
    ...
    full_path=$(find "$PARENT_DIR" -maxdepth 1 -type d -name "custom_*_ras" | head -n 1)
    if [ -n "$full_path" ]; then
    ...
    if $first_run; then
        ...
        dest_conf_ascend_first="$HOST_EXTRACT_DIR/opp/built-in/op_impl/ai_core/tbe/kernel/config/$ascend_dir/ops_ras"
        ...
        dest_conf_ascend="$HOST_EXTRACT_DIR/opp/built-in/op_impl/ai_core/tbe/kernel/config/$ascend_dir/ops_ras"
...
filelist_src_path="$HOST_EXTRACT_DIR/share/info/ops_ras/script/filelist.csv"
