...
# 包内路径
GRAPH_SO_PATH="${CURR_PATH}/../../../../opp/built-in/op_graph/lib/linux/${ARCH_INFO}/libopgraph_ras.so"
HOST_SO_PATH="${CURR_PATH}/../../../../opp/built-in/op_impl/ai_core/tbe/op_host/lib/linux/${ARCH_INFO}/libophost_ras.so"
...
judgment_path() {
  ...
  if [ $? -ne 0 ]; then
    echo "[OpsRas][ERROR]: The opp install path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
    exitlog
    exit 1
  fi
}
...
check_install_path() {
  ...
  if [ "x${TARGET_INSTALL_PATH}" = "x" ]; then
    echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --install-path\
...
get_opts() {
  ...
  if [ "$*" = "" ]; then
    echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:\
...
do not support overwriting installation!"
  ...
  if [ "${IS_UPGRADE}" = "y" ]; then
    if [ ! -e "${INSTALL_INFO_FILE}" ]; then
      logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${TARGET_INSTALL_PATH} not install OpsRas, upgrade failed."
      exitlog
      exit 1
    fi
...
install_package() {
  ...
  clean_before_reinstall
  if [ "$?" != 0 ]; then
    comm_log_operation "Install" "${IN_INSTALL_TYPE}" "OpsRas" "$?" "${CMD_LIST}"
  fi
...
uninstall_package() {
 ...
 installed in (${TARGET_VERSION_DIR}) and then set the correct install path."
    ...
    comm_log_operation "Uninstall" "${IN_INSTALL_TYPE}" "OpsRas" "$?" "${CMD_LIST}"
    exit 0
  fi
  ...
  # 删除异构so文件和空目录
  local graph_so_path="${TARGET_VERSION_DIR}/opp/built-in/op_graph/lib/linux/${target_arch}/libopgraph_ras.so"
  local host_so_path="${TARGET_VERSION_DIR}/opp/built-in/op_impl/ai_core/tbe/op_host/lib/linux/${target_arch}/libophost_ras.so"
