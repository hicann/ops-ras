...
opp_old_platform_dir=ops_ras_$platform_data-linux
opp_platform_dir=ops_ras
upper_opp_platform=$(echo "${opp_platform_dir}" | tr 'a-z' 'A-Z')
GRAPH_SO_PATH="${_CURR_PATH}/../../../../opp/built-in/op_graph/lib/linux/${platform_data}/libopgraph_ras.so"
HOST_SO_PATH="${_CURR_PATH}/../../../../opp/built-in/op_impl/ai_core/tbe/op_host/lib/linux/${platform_data}/libophost_ras.so"
# defaluts info determinated by user's inputs
_INSTALL_LOG_DIR="ops_ras/install_log"
...
startlog() {
    echo "[OpsRas] [$(getdate)] [INFO]: Start Time: $(getdate)"
}

exitlog() {
    echo "[OpsRas] [$(getdate)] [INFO]: End Time: $(getdate)"
}
...
${_cmd_type} ops_ras not support specific user name or user group. Please \
...
precleanbeforeinstall() {
    ...
    _files_existed=1
    # check the installation folder has files or ops_ras module existed or not
    ...
    if [ "${_files_existed}" = "0" ]; then
        if [ "${is_quiet}" = y ]; then
            logandprint "[WARNING]: Directory has file existed or installed ops_ras \
module, are you sure to keep installing ops_ras module in it? y"
        else
            if [ ! -f  "${_opp_sub_dir}""/ascend_install.info" ]; then
                logandprint "[INFO]: Directory has file existed, do you want to continue? [y/n]"
            else
                logandprint "[INFO]: OpsRas package has been installed on the path $(getinstalledinfo "${KEY_INSTALLED_PATH}"), \
...
and the version of this package is $(getrunpkginfo "${KEY_RUNPKG_VERSION}"), do you want to continue? [y/n]"
            ...
            do
                read yn
                if [ "$yn" = n ]; then
                    logandprint "[INFO]: Exit to install ops_ras module."
                    exitlog
                    exit 0
                elif [ "$yn" = y ]; then
                    break;
        ...
        logandprint "[INFO]: Directory is empty, directly install ops_ras module."
    fi
...
Please install without quiet mode and check permission."
    ...
    if [ "${_files_existed}" = "0" ] && [ "${_installed_path}" = "${_path}" ]; then
        logandprint "[INFO]: Clean the installed ops_ras module before install."
...
path or clean the previous version ops_ras install info (/etc/ascend_install.info) and then reinstall it."
            return 1
        fi
...
show_relation () {
    ...
    if [ "$relation_situation" = "SUCC" ] ;then
        logandprint "[INFO]: Relationship of ops_ras with ${req_pkg_name_val} in path ${req_pkg_path} checked successfully"
    else
        logandprint "[WARNING]: Relationship of ops_ras with ${req_pkg_name_val} in path ${req_pkg_path} checked failed."
    fi
    return
}
...
check_docker_path(){
    docker_path="$1"
    if [ "${docker_path}" != "/"* ]; then
        echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --docker-root \
must with absolute path that which is start with root directory /. Such as --docker-root=/${docker_path}"
        exitlog
        exit 1
    fi
    if [ ! -d "${docker_path}" ]; then
        echo "[OpsRas] [ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${docker_path} not exist, please create this directory."
        exitlog
        exit 1
    fi
}
...
judgment_path() {
    ...
    if [ $? -ne 0 ]; then
        echo "[OpsRas][ERROR]: The ops_ras install path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exitlog
        exit 1
    fi
}
...
check_install_path(){
    ...
    if [ "x${in_install_path_param}" = "x" ]; then
        echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter ${param_name} \
...
not support that the install path contains space character."
    ...
    if [ x"${prefix}" = "x" ]; then
        ...
        if [ x"${prefix}" = "x" ]; then
            ...
            echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES: Run package path is invalid: $run_path"
            exitlog
            exit 1
        fi
    fi
...
interact_pre_check() {
    ...
    if [ "$?" != 0 ]; then
        if [ "${is_quiet}" = y ]; then
            logandprint "[WARNING]: Precheck of ops_ras module execute failed! do you want to continue install? y"
        else
            logandprint "[WARNING]: Precheck of ops_ras module execute failed! do you want to continue install?  [y/n] "
            while true
            do
            read yn
            if [ "$yn" = "n" ]; then
                echo "stop install ops_ras module!"
                exit 1
            elif [ "$yn" = y ]; then
                break;
            else
...
switchchip(){
    if [ "$tmp_chip_type" = "Ascend910" ];then
       ...
       echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --chip \
        ...
        if [ ${num_0} -lt 7 ] || [ ${num_1} -lt 5 ] || [ ${num_2} -lt 5 ]; then
            ...
            if [ "${is_quiet}" = y ]; then
                ...
                do
                    read yn
                    if [ "$yn" = n ]; then
                        logandprint "[INFO]: stop install ops_ras module!"
                        exitlog
                        exit 1
                    elif [ "$yn" = y ]; then
                        break;
...
do
    ...
    --feature=*)
        ...
        if test -z "$feature_choice"; then
            echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Paramter --feature cannot be null."
            exitlog
            exit 1
        fi
    ...
    --setenv)
        ...
        ;;
    -*)
        echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Unsupported parameters [$1], \
...
if [ "${iter_i}" != 1 ]; then
    echo "[OpsRas] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:only support one type: full/run/devel/upgrade/uninstall/check, operation execute failed! \
...
if [ "$?" = "0" ]; then
    echo "[OpsRas] [$(getdate)] [INFO]: Uninstall the version before multi version of ops_ras successfully!"
else
    echo "[OpsRas] [$(getdate)] [ERROR]: Uninstall the version before multi version of ops_ras failed!"
    exit 1
fi
if [ "${is_input_path}" = y ]; then
...
_INSTALL_INFO_FILE="${target_dir}/share/info/${_INSTALL_INFO_SUFFIX}"
...
logandprint "[INFO]: Execute the ops_ras run package."
...
if [ "${is_check}"="y" ] && [ "${check_path}" != "" ]; then
    ...
    if [ ! -f "${_VERCHECK_FILE}" ]; then
        logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST};ERR_DES:The file\
(${_VERCHECK_FILE}) not exists. Please make sure that the ops_ras module\
...
if [ "${installed_version}" = "" ]; then
    logandprint "[INFO]: Version of installing ops_ras module is ${runpkg_version}."
else
    if [ "${runpkg_version}" != "" ]; then
        logandprint "[INFO]: Existed ops_ras module version is ${installed_version}, \
the new ops_ras module version is ${runpkg_version}."
    fi
fi
...
if [ "${is_check}" = "y" ]; then
    ...
    if [ $? -ne 0 ]; then
        logandprint "[ERROR]: ERR_NO:${OPP_COMPATIBILITY_CEHCK_ERR};ERR_DES:Check the compatibility of ops_ras package fail,please confirm the true version package."
        exit 1
    fi
fi
...
if [ "${is_install}" = "y" ];then
    ...
    # precheck before install ops_ras module
    ...
    if [ "${in_install_type}" != "devel" ]; then
        ...
        if [ "$?" != 0 ]; then
            logandprint "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Installation of ops_ras\
...
user group (${_DEFAULT_USERGROUP}) for devel mode? [y/n]"
    ...
    #getfirstnotexistdir "/usr/local/tmp/CANN-1.81/ops_ras"
    is_the_last_dir_opp=""
...
#            aicpuinfofile "remove"
    ...
    # check the compatibility of ops_ras
    ...
    # call opp_install.sh
    if [ -d "${target_dir}/ops_ras" ] && [ ! -L "${target_dir}/ops_ras" ] && [ -f "${target_dir}/ops_ras/script/uninstall.sh" ]; then
        bash "${target_dir}/ops_ras/script/uninstall.sh"
    fi
...
if [ "${is_upgrade}" = "y" ];then
    # if latest dir has no ops_ras module exit first!
    if [ "${is_for_all}" = y ] && [ $(id -u) -ne 0 ]; then
        check_dir_permission_for_common_user "${target_dir}"
    fi
    install_type=$(getinstalledinfo "${KEY_INSTALLED_TYPE}")
    # precheck before upgrade ops_ras module
    if [ "${is_precheck}" = "y" ];then
        interact_pre_check
    fi
    # check the compatibility of ops_ras
...
if [ "${is_uninstall}" = "y" ];then
    ...
    graph_so_path="${target_dir}/opp/built-in/op_graph/lib/linux/${target_arch}/libopgraph_ras.so"
    graph_so_dir_path="${target_dir}/opp/built-in/op_graph/lib/linux/${target_arch}"
    host_so_path="${target_dir}/opp/built-in/op_impl/ai_core/tbe/op_host/lib/linux/${target_arch}/libophost_ras.so"
