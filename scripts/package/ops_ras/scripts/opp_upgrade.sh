...
ops_ras_platform_old_dir=ops_ras_$platform_data-linux
ops_ras_platform_dir=ops_ras
upper_ops_ras_platform=$(echo "${ops_ras_platform_dir}" | tr 'a-z' 'A-Z')

_INSTALL_INFO_SUFFIX="share/info/${ops_ras_platform_dir}/ascend_install.info"
...
checkascendinfo() {
    ...
    if [ -f "${file_param}" ]; then
        ...
        if [ "$inst_type" != "" ]; then
            echo "${upper_ops_ras_platform}_INSTALL_TYPE=${inst_type}" >> ${file_param}
        fi
        if [ "$uname_param" != "" ]; then
        ...
        if [ "$path_param" != "" ]; then
                echo "${upper_ops_ras_platform}_INSTALL_PATH_VAL=${path_param}" >> ${file_param}
        fi
        if [ "$path_params" != "" ]; then
            echo "${upper_ops_ras_platform}_INSTALL_PATH_PARAM=${path_params}" >> ${file_param}
        fi

        if [ "$version_param" != "" ]; then
            echo "${upper_ops_ras_platform}_VERSION=${version_param}" >> ${file_param}
        fi
    fi
}
...
KEY_INSTALLED_TYPE="${upper_ops_ras_platform}_INSTALL_TYPE"
KEY_INSTALLED_FEATURE="${upper_ops_ras_platform}_Install_Feature"
KEY_INSTALLED_CHIP="${upper_ops_ras_platform}_INSTALL_CHIP"
KEY_INSTALLED_PATH="${upper_ops_ras_platform}_INSTALL_PATH_VAL"
KEY_INSTALLED_VERSION="${upper_ops_ras_platform}_VERSION"
...
[${_ugroup}] of ops_ras module is not right!"
        exit 1
    fi
}
...
createsoftlink() {
    ...
    _dst_path="$2"
    if [ -L "$2" ]; then
        logandprint "[WARNING]: Soft link for [ops_ras/ops] is existed. Cannot create new soft link."
        return 0
    fi
...
setenv() {
    logandprint "[INFO]: Set the environment path [ export ASCEND_OPS_RAS_PATH=${relative_path_val}/${ops_ras_platform_dir} ]."
...
get_package_upgrade_version_dir "upgrade_version_dir" "$_TARGET_INSTALL_PATH" "${ops_ras_platform_dir}"
...
# init log file path
_UNINSTALL_SHELL_FILE="${install_version_dir}""/share/info/${ops_ras_platform_dir}/script/opp_uninstall.sh"
# adpter for old version's path
if [ ! -f "${_UNINSTALL_SHELL_FILE}" ]; then
    _UNINSTALL_SHELL_FILE="${install_version_dir}""/share/info/${ops_ras_platform_dir}/scripts/opp_uninstall.sh"
fi
...
if [ "${install_type}" = "" ]; then
    logwitherrorlevel "1" "error" "[ERROR]: ERR_NO:${UPGRADE_FAILED};ERR_DES:OPS_RAS module\
...
# check the ops_ras module sub directory exist or not
opp_sub_dir="${_TARGET_INSTALL_PATH}/${upgrade_version_dir}""/share/info/${ops_ras_platform_dir}/"
old_opp_sub_dir="${_TARGET_INSTALL_PATH}/${upgrade_old_version_dir}""/share/info/${ops_ras_platform_old_dir}/"
...
logandprint "[INFO]: Begin upgrade ops_ras module."
logandprint "[INFO]: Uninstall the Existed ops_ras module before upgrade."
# call uninstall functions
if [ "${_IS_ADAPTER_MODE}" = "true" ]; then
    if [ ! -f "${_UNINSTALL_SHELL_FILE}" ]; then
        logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST};ERR_DES:The file\
(${_UNINSTALL_SHELL_FILE}) not exists. Please make sure that the ops_ras module\
 installed in (${install_version_dir}) and then set the correct install path."
    ...
    if [ "$pkg_is_multi_version" = "true" ] && [ "${is_upgrade}" = "y" ]; then
        if [ -f "${_TARGET_INSTALL_PATH}/${upgrade_old_version_dir}/${ops_ras_platform_old_dir}/script/uninstall.sh" ]; then
            ${_TARGET_INSTALL_PATH}/${upgrade_old_version_dir}/${ops_ras_platform_old_dir}/script/uninstall.sh
            if [ "$?" = 0 ]; then
                logandprint "[INFO]: Uninstall last ops_ras version module successfully."
            else
                logandprint "[ERROR]: Uninstall last ops_ras version module fail."
                exit 1
            fi
        elif [ -f "${_TARGET_INSTALL_PATH}/${upgrade_version_dir}/ops_ras/script/uninstall.sh" ]; then
            ${_TARGET_INSTALL_PATH}/${upgrade_version_dir}/ops_ras/script/uninstall.sh
            if [ "$?" = 0 ]; then
                logandprint "[INFO]: Uninstall last ops_ras version module successfully."
            else
                logandprint "[ERROR]: Uninstall last ops_ras version module fail."
...
logandprint "[INFO]: Update the ops_ras install info."
...
logwitherrorlevel "$?" "error" "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Update ops_ras install info failed."

logandprint "[INFO]: upgradePercentage:30%"
# create copy and chmod ops_ras module path
logandprint "[INFO]: Install ops_ras module path in the install folder."
setenv

sh "${_COMMON_PARSER_FILE}" --package="${ops_ras_platform_dir}" --install --username="${_TARGET_USERNAME}" --usergroup="${_TARGET_USERGROUP}" --set-cann-uninstall \
...
logwitherrorlevel "$?" "error" "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Install ops_ras module files failed."
logandprint "[INFO]: upgradePercentage:50%"

# change installed folder's permission except aicpu
if [ -f "${install_version_dir}/${ops_ras_platform_dir}" ]; then
    subdirs_info=$(ls "${install_version_dir}/${ops_ras_platform_dir}" 2> /dev/null)
    for dir in ${subdirs_info}; do
        if [ "${dir}" != "Ascend310" ] && [ "${dir}" != "Ascend310RC" ] && [ "${dir}" != "Ascend910" ] && [ "${dir}" != "Ascend310P" ] && [ "${dir}" != "Ascend" ] &&  [ "${dir}" != "aicpu" ] && [ "${dir}" != "script" ]; then
            chmod -R "${_CUSTOM_PERM}" "${install_version_dir}/ops_ras/${dir}" 2> /dev/null
        fi
    done
    chmod "${_CUSTOM_PERM}" "${install_version_dir}/${ops_ras_platform_dir}" 2> /dev/null
...
# change installed folder's permission except aicpu
subdirs=$(ls "${install_version_dir}/${ops_ras_platform_dir}" 2> /dev/null)
for dir in ${subdirs}; do
    if [ "${dir}" != "vendors" ] && [ "${dir}" != "Ascend310" ] && [ "${dir}" != "Ascend310RC" ] && [ "${dir}" != "Ascend910" ] && [ "${dir}" != "Ascend310P" ] && [ "${dir}" != "Ascend" ] &&  [ "${dir}" != "aicpu" ] && [ "${dir}" != "script" ] && [ "${dir}" != "static_kernel" ]; then
        chmod -R "${_BUILTIN_PERM}" "${install_version_dir}/${ops_ras_platform_dir}/${dir}" 2> /dev/null
    fi
done

if [ "$(id -u)" = "0" ]; then
    chmod "755" "${install_version_dir}/${ops_ras_platform_dir}" 2> /dev/null
else
    chmod "${_BUILTIN_PERM}" "${install_version_dir}/${ops_ras_platform_dir}" 2> /dev/null
fi

vendor_dir=$(ls "${install_version_dir}/${ops_ras_platform_dir}/vendors" 2> /dev/null)
if [ -d "$vendor_dir" ]; then
    chmod -R "${_CUSTOM_PERM}" "${install_version_dir}""/${ops_ras_platform_dir}/vendors/$vendor_dir/framework/" 2> /dev/null
    chmod -R "${_CUSTOM_PERM}" "${install_version_dir}""/${ops_ras_platform_dir}/vendors/$vendor_dir/fusion_pass/" 2> /dev/null
    chmod -R "${_CUSTOM_PERM}" "${install_version_dir}""/${ops_ras_platform_dir}/vendors/$vendor_dir/fusion_rules/" 2> /dev/null
    chmod -R "${_CUSTOM_PERM}" "${install_version_dir}""/${ops_ras_platform_dir}/vendors/$vendor_dir/op_impl/" 2> /dev/null
    chmod -R "${_CUSTOM_PERM}" "${install_version_dir}""/${ops_ras_platform_dir}/vendors/$vendor_dir/op_proto/" 2> /dev/null
fi
chmod "${_ONLYREAD_PERM}" "${install_version_dir}""/${ops_ras_platform_dir}/scene.info" 2> /dev/null
chmod "${_ONLYREAD_PERM}" "${install_version_dir}""/${ops_ras_platform_dir}/version.info" 2> /dev/null
chmod 600 "${install_version_dir}""/${ops_ras_platform_dir}/ascend_install.info" 2> /dev/null
...
for dir in ${subdirs}; do
    if [ "${dir}" != "Ascend310" ] && [ "${dir}" != "Ascend310RC" ] && [ "${dir}" != "Ascend910" ] && [ "${dir}" != "Ascend310P" ] && [ "${dir}" != "Ascend" ] && [ "${dir}" != "aicpu" ] && [ "${dir}" != "script" ]; then
        chown -R "${_TARGET_USERNAME}":"${_TARGET_USERGROUP}" "${install_version_dir}/ops_ras/${dir}" 2> /dev/null
    fi
done

#chmod to support copy
if [ -d "${install_version_dir}/${ops_ras_platform_dir}/vendors" ] && [ "$(id -u)" != "0" ]; then
    chmod -R "${_CUSTOM_PERM}" ${install_version_dir}/${ops_ras_platform_dir}/vendors
fi
