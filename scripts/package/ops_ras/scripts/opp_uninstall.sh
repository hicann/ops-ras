...
uninstall_es_whl() {
    ...
    if [ ! -d "${whl_install_dir_path}/es_ras" ] &&
       ! compgen -G "${whl_install_dir_path}/es_ras-*.dist-info" > /dev/null; then
        return 0
    fi

    chmod u+w "${whl_install_dir_path}" 2> /dev/null
    chmod u+w -R "${whl_install_dir_path}"/es_ras 2> /dev/null
    chmod u+w -R "${whl_install_dir_path}"/es_ras-*.dist-info 2> /dev/null
    whl_uninstall_package "es_ras" "${whl_install_dir_path}"
}

uninstall_cann_ops_ras_whl() {
    local python_dir="${TARGET_VERSION_DIR}/python/site-packages"

    if [ -d "${python_dir}/cann_ops_ras" ]; then
        logandprint "[INFO]: Removing cann_ops_ras whl package from ${python_dir}"

        if command -v pip3 &>/dev/null; then
            pip3 uninstall -y cann_ops_ras --target="${python_dir}" 2>/dev/null
        fi

        rm -rf "${python_dir}/cann_ops_ras" 2>/dev/null
        rm -f ${python_dir}/cann_ops_ras-*.whl 2>/dev/null
        rm -rf ${python_dir}/cann_ops_ras-*.egg-info 2>/dev/null
        rm -rf ${python_dir}/cann_ops_ras-*.dist-info 2>/dev/null
        ...
        logandprint "[INFO]: cann_ops_ras whl package removed"
    fi
}
