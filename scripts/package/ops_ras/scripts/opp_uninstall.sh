...
unsetenv() {
    logandprint "[INFO]: Unset the environment path [ export ASCEND_OPS_RAS_PATH=${relative_path_val}/${ops_ras_platform_dir} ]."
...
logandprint "[INFO]: Command ops_ras_uninstall"
...
upper_opp_platform=$(echo "${ops_ras_platform_dir}" | tr 'a-z' 'A-Z')
...
# this is ops_ras verion info file
...
logandprint "[INFO]: Begin uninstall the ops_ras module."

# check install folder existed
checkfileexist "${_INSTALL_INFO_FILE}"
logwitherrorlevel "$?" "error" "[ERROR]: ERR_NO:${OPERATE_FAILED};ERR_DES:Uninstall ops_ras module failed."
checkfileexist "${_FILELIST_FILE}"
logwitherrorlevel "$?" "error" "[ERROR]: ERR_NO:${OPERATE_FAILED};ERR_DES:Uninstall ops_ras module failed."
checkfileexist "${_COMMON_PARSER_FILE}"
logwitherrorlevel "$?" "error" "[ERROR]: ERR_NO:${OPERATE_FAILED};ERR_DES:Uninstall ops_ras module failed."
ops_ras_sub_dir="${_ABS_INSTALL_PATH}""/share/info/${ops_ras_platform_dir}"
checkdirectoryexist "${ops_ras_sub_dir}"
logwitherrorlevel "$?" "error" "[ERROR]: ERR_NO:${OPERATE_FAILED};ERR_DES:Uninstall ops_ras module failed."
...
subdirs=$(ls "${_TARGET_INSTALL_PATH}/${ops_ras_platform_dir}" 2> /dev/null)
for dir in ${subdirs}; do
    if [ "${dir}" != "Ascend310" ] && [ "${dir}" != "Ascend310RC" ] && [ "${dir}" != "Ascend910" ] && [ "${dir}" != "Ascend310P" ] && [ "${dir}" != "Ascend" ] && [ "${dir}" != "aicpu" ]; then
        chmod -R "${_CUSTOM_PERM}" "${_TARGET_INSTALL_PATH}/${ops_ras_platform_dir}/${dir}" 2> /dev/null
    fi
done
chmod "${_CUSTOM_PERM}" "${_TARGET_INSTALL_PATH}/${ops_ras_platform_dir}" 2> /dev/null

remove_init_py() {
  local built_in_impl_path=${_ABS_INSTALL_PATH}/opp/built-in/op_impl/ai_core/tbe/impl/ops_ras
...
sh "${_COMMON_PARSER_FILE}" --package="${ops_ras_platform_dir}" --uninstall --recreate-softlink --username="${target_username}" --usergroup="${target_usergroup}" --version=$pkg_version \
...
if [ "${uninstall_mode}" != "upgrade" ]; then
    ...
    logwitherrorlevel "$?" "warn" "[WARNING]Delete ops_ras install info file failed, \
...
# delete the empty ops_ras folder it'self
res_val=$(ls "${ops_ras_sub_dir}" 2> /dev/null)
if [ "${res_val}" = "" ]; then
    rm -rf -d "${ops_ras_sub_dir}" >> /dev/null 2>&1
fi

# change installed folder's permission except aicpu
subdirs_param=$(ls "${_ABS_INSTALL_PATH}/${ops_ras_platform_dir}" 2> /dev/null)
for dir in ${subdirs_param}; do
    if [ "${dir}" != "Ascend310" ] && [ "${dir}" != "Ascend310RC" ] && [ "${dir}" != "Ascend910" ] && [ "${dir}" != "Ascend310P" ] && [ "${dir}" != "Ascend" ] && [ "${dir}" != "aicpu" ]; then
        chmod "${_BUILTIN_PERM}" "${_ABS_INSTALL_PATH}/${ops_ras_platform_dir}/${dir}" 2> /dev/null
    fi
done
...
temp=$(ls "${_ABS_INSTALL_PATH}/${ops_ras_platform_dir}" 2> /dev/null)
if [ -d "${_ABS_INSTALL_PATH}/${ops_ras_platform_dir}/" ]; then
    # find custom file in path and print log
    for file in $(ls -A ${_ABS_INSTALL_PATH}/${ops_ras_platform_dir}/* 2> /dev/null)
    do
...
# delete scene.info
scene_dir="${_ABS_INSTALL_PATH}/${ops_ras_platform_dir}/scene.info"
if [ -f ${scene_dir} ]; then
    rm -f ${scene_dir}
fi
...
logandprint "[INFO]: OPS_RAS package uninstalled successfully! Uninstallation takes effect immediately."
exit 0
