...
install_es_whl() {
  local es_whl_path="${SOURCEDIR}/es_packages/whl/es_ras-1.0.0-py3-none-any.whl"
  if [ ! -f "${es_whl_path}" ]; then
    logandprint "[INFO]: No RAS Graph/ES wheel found, skip es_ras installation."
    return 0
  fi
  ...
  if [ -d "${TARGET_VERSION_DIR}/ops_ras" ]; then
    rm -rf "${TARGET_VERSION_DIR}/ops_ras"
  fi
}

install_cann_ops_ras_whl() {
  local whl_dir="${CURR_PATH}/../../../../python/site-packages"
  local target_python_dir="${TARGET_VERSION_DIR}/python/site-packages"

  local whl_file=$(find "${whl_dir}" -name "cann_ops_ras-*.whl" 2>/dev/null | head -1)

  if [ -n "${whl_file}" ] && [ -f "${whl_file}" ]; then
    logandprint "[INFO]: Found whl package: ${whl_file}"
    logandprint "[INFO]: Installing cann_ops_ras whl package to ${target_python_dir}"
    ...
    if command -v pip3 &>/dev/null; then
      if pip3 install --target="${target_python_dir}" "${whl_file}" --no-deps --force-reinstall 1>/dev/null 2>&1; then
        pip_installed=true
      fi
    fi

    if [ "${pip_installed}" = "false" ] && command -v pip &>/dev/null; then
      if pip install --target="${target_python_dir}" "${whl_file}" --no-deps --force-reinstall 1>/dev/null 2>&1; then
        pip_installed=true
      fi
    fi
    ...
    logandprint "[INFO]: cann_ops_ras whl package installed successfully"
  else
    logandprint "[INFO]: No cann_ops_ras whl package found, skipping"
  fi
}
