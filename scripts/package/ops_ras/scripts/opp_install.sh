...
_VERSION_INFO_SUFFIX="share/info/${ops_ras_platform_dir}/version.info"
...
logandprint "[INFO]: Command ops_ras_install"
...
get_package_upgrade_version_dir "upgrade_old_version_dir" "$_TARGET_INSTALL_PATH" "${ops_ras_platform_old_dir}"
get_package_last_installed_version "last_installed" "$_TARGET_INSTALL_PATH" "${ops_ras_platform_dir}"
get_package_last_installed_version "last_installed_old" "$_TARGET_INSTALL_PATH" "${ops_ras_platform_old_dir}"
...
logandprint "[INFO]: Begin install ops_ras module."
...
add_init_py() {
    ...
    local built_in_impl_path=${target_built_in}/op_impl/ai_core/tbe/impl/ops_ras
...
if [ "${_FIRST_NOT_EXIST_DIR}" != "" ]; then
    mkdir -p "${version_install_dir}/${ops_ras_platform_dir}" 2> /dev/null
...
# make the ops_ras and the upper folder can write files
...
subdirs_info=$(ls "${version_install_dir}/${ops_ras_platform_dir}" 2> /dev/null)
for dir in ${subdirs_info}; do
    if [ "${dir}" != "Ascend310" ] && [ "${dir}" != "Ascend310RC" ] && [ "${dir}" != "Ascend910" ] && [ "${dir}" != "Ascend310P" ] && [ "${dir}" != "Ascend" ]  &&  [ "${dir}" != "aicpu" ] && [ "${dir}" != "script" ]; then
        chmod -R "${_CUSTOM_PERM}" "${version_install_dir}/${ops_ras_platform_dir}/${dir}" 2> /dev/null
    fi
done
chmod "${_CUSTOM_PERM}" "${version_install_dir}/${ops_ras_platform_dir}" 2> /dev/null

comm_create_dir "${version_install_dir}/share/info/${ops_ras_platform_dir}" "${_CREATE_DIR_PERM}" "${_TARGET_USERNAME}:${_TARGET_USERGROUP}" "${is_for_all}"
...
if [ -d "${version_install_dir}/${ops_ras_platform_dir}/vendors" ] && [ "$(id -u)" != "0" ]; then
    chmod -R "${_CUSTOM_PERM}" ${version_install_dir}/${ops_ras_platform_dir}/vendors
fi
...
if [ "$(id -u)" = "0" ]; then
    chmod "755" "${version_install_dir}/${ops_ras_platform_dir}" 2> /dev/null
else
    chmod "${_BUILTIN_PERM}" "${version_install_dir}/${ops_ras_platform_dir}" 2> /dev/null
fi

chmod "${_ONLYREAD_PERM}" "${version_install_dir}""/share/info/${ops_ras_platform_dir}/scene.info" 2> /dev/null
chmod "${_ONLYREAD_PERM}" "${version_install_dir}""/share/info/${ops_ras_platform_dir}/version.info" 2> /dev/null
chmod 600 "${version_install_dir}""/share/info/${ops_ras_platform_dir}/ascend_install.info" 2> /dev/null

if [ "${is_change_dir_mode}" = "true" ]; then
    chmod u-w "${version_install_dir}" 2> /dev/null
fi

rm -fr ${version_install_dir}/${ops_ras_platform_dir}
...
logandprint "[INFO]: OPS_RAS package installed successfully! The new version takes effect immediately."
exit 0
