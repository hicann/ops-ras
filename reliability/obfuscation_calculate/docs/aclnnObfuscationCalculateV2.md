...
- `aclnnStatus aclnnObfuscationCalculateV2GetWorkspaceSize(int32_t fd, const aclTensor* x, int32_t param, int32_t cmd, float obfCoefficient, aclTensor* y, uint64_t* workspaceSize, aclOpExecutor** executor)`
- `aclnnStatus aclnnObfuscationCalculateV2(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream)`

## aclnnObfuscationCalculateV2GetWorkspaceSize

* **参数说明**：

  * fd（int32_t ，计算输入）：socket连接符，数据类型为INT32，填写aclnnObfuscationSetupV2在资源初始化时输出中的fd[0]。
  * x（aclTensor* ，计算输入）：待混淆处理的张量，Shape为(*,*,...,hiddenSize)，最后一维的size是aclnnObfuscationSetupV2在资源初始化时配置的hiddenSize，[数据格式](../../../docs/zh/context/数据格式.md)要求为ND。不支持[非连续的Tensor](../../../docs/zh/context/非连续的Tensor.md)，不支持空Tensor。
  ...
  * obfCoefficient（float，计算输入）：进行混淆处理使用的混淆系数。取值范围(0.0， 1.0]。
...
* **参数说明：**
  ...
  * workspaceSize（uint64_t，入参）：在Device侧申请的workspace大小，由第一段接口aclnnObfuscationCalculateV2GetWorkspaceSize获取。
