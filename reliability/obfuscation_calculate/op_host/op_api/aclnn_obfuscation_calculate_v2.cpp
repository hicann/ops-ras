...
static aclnnStatus CheckParams(const int32_t fd, const aclTensor* x, const int32_t param, 
                              const int32_t cmd, float obfCoefficient, const aclTensor* y)
{
  // 1. 检查参数是否为空指针
...
aclnnStatus aclnnObfuscationCalculateV2GetWorkspaceSize(int32_t fd, const aclTensor* x, int32_t param,
                                                      int32_t cmd, float obfCoefficient, aclTensor* y, 
                                                      uint64_t* workspaceSize, aclOpExecutor** executor)
{
  L2_DFX_PHASE_1(aclnnObfuscationCalculateV2, DFX_IN(fd, x, param, cmd), DFX_OUT(y));
  // 固定写法，创建OpExecutor
  auto uniqueExecutor = CREATE_EXECUTOR();
  CHECK_RET(uniqueExecutor.get() != nullptr, ACLNN_ERR_INNER_CREATE_EXECUTOR);

  // 固定写法，参数检查
  auto ret = CheckParams(fd, x, param, cmd, obfCoefficient, y);
...
{
  L2_DFX_PHASE_2(aclnnObfuscationCalculateV2);
  // 固定写法，调用框架能力，完成计算
