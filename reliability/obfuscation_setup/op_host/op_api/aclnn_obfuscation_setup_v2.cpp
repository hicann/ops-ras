...
static aclnnStatus CheckParams(const int32_t fdToClose, const int32_t dataType,
                               ...
                               const int32_t cmd, const int32_t threadNum, float obfCoefficient, const aclTensor* fd)
{
  // 1. 检查参数是否为空指针
...
aclnnStatus aclnnObfuscationSetupV2GetWorkspaceSize(int32_t fdToClose, int32_t dataType, 
                                                  int32_t hiddenSize, int32_t tpRank,
                                                  int32_t modelObfSeedId, int32_t dataObfSeedId, 
                                                  int32_t cmd, int32_t threadNum, float obfCoefficient, aclTensor* fd, 
                                                  uint64_t* workspaceSize, aclOpExecutor** executor)
{
  L2_DFX_PHASE_1(aclnnObfuscationSetupV2, DFX_IN(fdToClose, dataType, hiddenSize, tpRank, modelObfSeedId, dataObfSeedId, cmd, threadNum, obfCoefficient), DFX_OUT(fd));
  // 固定写法，创建OpExecutor
  auto uniqueExecutor = CREATE_EXECUTOR();
  CHECK_RET(uniqueExecutor.get() != nullptr, ACLNN_ERR_INNER_CREATE_EXECUTOR);

  // 固定写法，参数检查
  auto ret = CheckParams(fdToClose, dataType, hiddenSize, tpRank, modelObfSeedId,dataObfSeedId, cmd, threadNum, obfCoefficient, fd);
...
{
  L2_DFX_PHASE_2(aclnnObfuscationSetupV2);
  // 固定写法，调用框架能力，完成计算
