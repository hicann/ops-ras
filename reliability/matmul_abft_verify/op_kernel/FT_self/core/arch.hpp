...
#ifndef FTSELF_CORE_ARCH_HPP
#define FTSELF_CORE_ARCH_HPP
...
template <AscendC::TPosition Position>
using PositionType = std::integral_constant<AscendC::TPosition, Position>;
...
struct LocalTensorBuffer {
    ...
    FTSELF_DEVICE LocalTensorBuffer()
    {
        AscendC::TBuf<Position> buffer;
        constexpr uint32_t size = Position == AscendC::TPosition::A1 || Position == AscendC::TPosition::B1 ||
            Position == AscendC::TPosition::C1 ? ArchTag::L1_SIZE :
            Position == AscendC::TPosition::A2 ? ArchTag::L0A_SIZE :
            Position == AscendC::TPosition::B2 ? ArchTag::L0B_SIZE :
            Position == AscendC::TPosition::C2 ? ArchTag::BIAS_SIZE :
            Position == AscendC::TPosition::CO1 ? ArchTag::L0C_SIZE :
            Position == AscendC::TPosition::C2PIPE2GM ? ArchTag::FIXBUF_SIZE : ArchTag::UB_SIZE;
        GetTPipePtr()->InitBuffer(buffer, size);
        tensor = buffer.template Get<uint8_t>();
    }

    template <class Element = half>
    FTSELF_DEVICE AscendC::LocalTensor<Element> GetBufferByByte(uint32_t offset) const
    {
...
struct Resource {
    ...
    FTSELF_DEVICE Resource() { pipe.Destroy(); }
};
