...
usage() {
  ...
  echo "build script for ops-ras repository"
  echo "Usage:"
...
build_static_lib() {
  ...
  if grep -wq "ophost_ras_static" <<< "${all_targets}"; then
    cmake --build . --target ophost_ras_static -- ${VERBOSE} -j $THREAD_NUM
  fi
  ...
  cmake --build . --target opapi_ras_static -- ${VERBOSE} -j $THREAD_NUM
  local jit_command=""
  if [[ "$ENABLE_JIT" == "TRUE" ]]; then
    jit_command="-j"
  fi
  ...
  cmake --build . --target cann_ras_static -- ${VERBOSE} -j $THREAD_NUM
...
package_static() {
    ...
    if [[ "$run_file" != *"ops-ras"* ]]; then
        echo "Error: Filename '$run_file' does not contain 'ops-ras'."
        return 1
    fi
    local new_name="${run_file/ops-ras/ops-ras-static}"
