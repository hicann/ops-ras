...
- **ops-ras包**：
  ...
  选择整个项目编译生成的包称为ops-ras包，可**完整替换**CANN包对应部分。该包支持aclnn和图模式调用AI Core算子。

- **ops-ras静态库**：

  > 说明：若您需要**基于本项目进行二次发布**并且对**软件包大小有要求**时，建议采用静态库编译，该库可以链接您的应用开发程序，仅保留业务所需的算子，从而实现软件最小化部署。

  表示整个项目编译为一个静态库文件，包含libcann_ras_static.a和aclnn接口头文件。该包仅支持aclnn调用AI Core算子。
...
1. **编译自定义算子包**
    ...
    # 编译experimental贡献目录下的用户算子（以AddExample算子为例，编译时请以实际贡献算子为准）
